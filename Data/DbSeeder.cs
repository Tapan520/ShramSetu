...
public static class DbSeeder
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        // Roles
        foreach (var role in new[] { "SuperAdmin", "Admin", "Employer", "Worker" })
            if (!await roleManager.RoleExistsAsync(role))
                await roleManager.CreateAsync(new IdentityRole(role));

        // SuperAdmin User
        const string superAdminEmail = "superadmin@shramsetu.in";
        if (await userManager.FindByEmailAsync(superAdminEmail) is null)
        {
            ...
            if ((await userManager.CreateAsync(superAdmin, "SuperAdmin@12345")).Succeeded)
